...
export class QuayApi {
  ...
  constructor(config: Config, logger: LoggerService) {
    this.apiUrl =
      config.getOptionalString('quay.apiBaseUrl') || DEFAULT_PROXY_PATH;
    this.token = config.getOptionalString('quay.apiKey');
    this.logger = logger;
  }

  static fromConfig(config: Config, logger: LoggerService): QuayApi {
    return new QuayApi(config, logger);
  }
  ...
  private async fetchFromQuay(endpoint: string): Promise<any> {
    ...
    try {
      ...
      if (!response.ok) {
        this.logger.error(`Quay API request failed: ${response.statusText}`);
        throw new Error(`Failed to fetch data: ${response.statusText}`);
      }

      return await response.json();
    } catch (error) {
      this.logger.error(`Quay API error: ${error}`);
      throw new Error(`Quay Api request failed: ${error}`);
    }
  }
